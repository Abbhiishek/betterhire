...
import os from 'os';
import { ResumeExtractPrompt } from '@/data/prompt';
import { ResumeType } from '@/data/resume';
import { auth } from '@/lib/auth';
import { populateUser } from '@/actions/populate-user';
...
export async function POST(request: NextRequest) {
    try {
        ...
        const prompt = ResumeExtractPrompt;
        ...
        let reviewJson;
        try {
            reviewJson = JSON.parse(result.response.text()) as ResumeType;
            populateUser(reviewJson)
        } catch (error) {
